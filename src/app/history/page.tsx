...
export default function HistoryPage() {
  ...
  return (
    <div>
      <div className="mb-8">
        <h1 className="text-2xl font-bold text-gray-900">Historikk</h1>
        <p className="text-sm text-gray-500 mt-1">{history.length} innholdselementer generert</p>
      </div>

      <div className="flex gap-3 mb-6 flex-wrap">
        <select value={filterClient} onChange={e => setFilterClient(e.target.value)}
          className="bg-white border border-gray-200 rounded-lg px-3 py-2 text-sm text-gray-700 focus:border-indigo-500 focus:outline-none">
            ...
            <div key={h.id} className="bg-white rounded-xl border border-gray-100 shadow-sm overflow-hidden">
              <button onClick={() => setExpanded(expanded === h.id ? null : h.id)}
                ...
                <div className="flex items-center gap-3">
                  {h.agents?.length > 0 && <span className="text-[10px] text-gray-400">{h.agents.length} agenter</span>}
