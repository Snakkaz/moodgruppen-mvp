    ...
    <div>
      ...
      {filtered.length === 0 ? (
        <div className="bg-[#12121a] border border-white/5 rounded-xl p-8 text-center">
          <p className="text-white/40 text-sm">{history.length === 0 ? "Ingen innhold generert ennå" : "Ingen treff med valgte filtre"}</p>
        </div>
      ) : (
            ...
            <div key={h.id} className="bg-[#12121a] border border-white/5 rounded-xl overflow-hidden">
              <button onClick={() => setExpanded(expanded === h.id ? null : h.id)}
                ...
                <div className="flex items-center gap-3">
                  ...
                  <span className="text-xs text-white/30">{new Date(h.createdAt).toLocaleDateString("nb-NO")}</span>
